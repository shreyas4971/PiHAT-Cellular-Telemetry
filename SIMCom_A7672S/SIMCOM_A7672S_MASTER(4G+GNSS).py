# ==========================================================
# SIMCOM A7672S "FULL AUTO WITH GNSS ENABLED" FRAMEWORK
# ==========================================================
# Features:
...
def send_mqtt_payload(ser, command, text):
    try:
        ...
        time.sleep(2.0) # ADDED DELAY: Stop modem from dropping first packet
...
def ensure_pdp(ser, retry_count=[0]):
    ...
    for _ in range(3):
        ...
        if "ERROR" not in act_resp and act_resp.strip():
            ...
            time.sleep(8) # ADDED DELAY: Increased from 5 to 8 to let PDP context lock
            return True
...
def build_mqtt_from_scratch(ser):
    ...
    time.sleep(3) # ADDED DELAY: Allow MQTT engine to start
    
    client_id = f"pi_hat_{int(time.time())}"
    send_at(ser, f'AT+CMQTTACCQ=0,"{client_id}"', 2)
    time.sleep(3) # ADDED DELAY: Allow Client ID to register
